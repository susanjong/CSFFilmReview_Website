...
include 'delete_jurassicpark.php';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if (!empty($komentar)) {
            ...
            $query = "INSERT INTO thelostworld_review (komentar, bintang, user_id) VALUES (:komentar, :bintang, :user_id)";
...
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
    $query = "SELECT er.*, u.username FROM thelostworld_review er JOIN users u ON er.user_id = u.id ORDER BY er.tanggal DESC";
    $stmt = $conn->prepare($query);
    $stmt->execute();
...
try {
    $avgQuery = "SELECT AVG(bintang) AS rata_rata_bintang FROM thelostworld_review";
...
<head>
    ...
    <link rel="stylesheet" href="/Film/The Lost World- Jurassic Park/jurassicpark.css">
</head>
...
<header>
    ...
    <div class="main-content">
        ...
        <div class="details">
            <h1>THE LOST WORLD:<br> JURASSIC PARK</h1>
            <div class="year">1997, Steven Spielberg, <br> Michael Crichton, David Koep </div>
            ...
            <br>
            <p class="synopsis">
            Malcolm (Jeff Goldblum) meets Hammond's (Richard Attenborough) nephew Peter Ludlow (Arliss Howard), the new CEO of Ingen who accuses Malcolm of spreading false rumors about Isla Nublar for payment. Peter says that Malcolm signed an NDA and then proceeded to violate that agreement. As such, his tenure at the university was rescinded and his reputation was left in tatters for spreading unsubstantiated rumors about Isla Nublar. 
            Hammond tells Malcolm that site B was the breeding site for Dinosaurs (Hammond calls Sorna the Factory floor, while Nublar is referred to as the showroom) & after hurricane Clarrisa the Dinos were left free to roam. Hammond says that for 4 years the dinosaurs have lived in a natural environment without human intervention, in their own social groups.
    ...
    <div class="reviews">
        ...
        <div id="reviewForm">
            ...
            <section class="left-form" style="width:35%;">
                <img src="/Photos/thelostworld.webp" style="width:220px;border-radius:20px;margin-top:30px;">
            </section>

            <section class="right-form">
                <h2 style="margin-top: 20px;">I've watched..</h2>
                <h1 style="font-family:Oswald;font-size:35px;font-weight: 700;text-shadow: 1px 1px 1px black;">The Lost World: <br> Jurassic Park
                <span style="font-size:25px;font-weight:400;font-family:Oswald;text-shadow: 1px 1px 1px black;color:#b8dbff">&nbsp1997</span>
                </h1>
