...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if (!empty($komentar)) {
            ...
            $query = "INSERT INTO thering_review (komentar, bintang, user_id) VALUES (:komentar, :bintang, :user_id)";
...
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
    $query = "SELECT er.*, u.username FROM thering_review er JOIN users u ON er.user_id = u.id ORDER BY er.tanggal DESC";
    $stmt = $conn->prepare($query);
    $stmt->execute();
...
try {
    $avgQuery = "SELECT AVG(bintang) AS rata_rata_bintang FROM thering_review";
...
<head>
    ...
    <link rel="stylesheet" href="/Film/The Ring/thering.css">
</head>
...
<header>
    ...
    <div class="main-content">
        ...
        <div class="details">
            <h1>THE RING </h1>
            <div class="year">2002, Gore Verbinski, Kōji <br> Suzuki, Ehren Kruger</div>
            ...
            <br>
            <p class="synopsis">
            The story begins with two teenage girls discussing the events of the previous weekend, during which one of them, Katie Embry (played by Amber Tamblyn), went to a cabin in the mountains to spend time with some friends. While talking, the subject of a supposedly cursed videotape is brought up. The other girl, Rebecca 'Becca' Kotler (played by Rachael Bella), states that anyone who watches this video receives a phone call, in which a voice says, "you will die in seven days." Then, exactly seven days (to the minute) after viewing the tape, the viewer dies. Katie reveals in horror that she had watched that video at the cabin last weekend with three friends, exactly seven days earlier.
    ...
    <div class="reviews">
        ...
        <div id="reviewForm">
            ...
            <section class="left-form" style="width:35%;">
                <img src="/Photos/thering.jpg" style="width:220px;border-radius:20px;margin-top:30px;">
            </section>

            <section class="right-form">
                <h2 style="margin-top: 20px;">I've watched..</h2>
                <h1 style="font-family:Oswald;font-size:35px;font-weight: 700;text-shadow: 1px 1px 1px black;">The Ring
                <span style="font-size:25px;font-weight:400;font-family:Oswald;text-shadow: 1px 1px 1px black;color:#b8dbff">&nbsp2002</span>
                </h1>
